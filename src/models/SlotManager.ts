...
export interface ISlotManager {
  splitIntoAvailableSlots(slots: Slot[], duration: number, dateISO: string, durationBefore: number, durationAfter: number): AvailableSlot[];
...
export class SlotManager implements ISlotManager {
  ...
  constructor(private dateHandler: DateHandler) {}

  splitIntoAvailableSlots(slots: Slot[], duration: number, dateISO: string, durationBefore: number, 
    durationAfter: number): AvailableSlot[] {
    const availableSlots: AvailableSlot[] = [];
    slots.forEach(slot => {
      let start = slot.start;
      let resultSlot: AvailableSlot | null;
      do {
        resultSlot = this.createAvailableSlot(start, slot.end, duration, dateISO, durationBefore, durationAfter);
        if (resultSlot) {
  ...
  private createAvailableSlot(
    ...
    const endHour = addMinutes(startHour, duration + durationBefore + durationAfter);
